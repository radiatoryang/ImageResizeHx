...
class ImageResize
{
	...
	public function scale(source:Pixels, ratio:Float)
	{
		...
		var valuesOffset:Int = source.bytes.length;
		var outputOffset:Int = valuesOffset + imageSize;

		source.convertTo(PixelFormat.ARGB);

		var vramLength:Int = source.bytes.length + imageSize * 8 + imageSize * 4;
		var vram = Bytes.alloc(vramLength);
		vram.blit(0, source.bytes, 0, source.bytes.length);
			...
			for (x in 0...width)
			{
				...
				for (y1 in y1b...y1e)
				{
					...
					for (x1 in x1b...x1e)
					{
						...
						vram.setDouble(valuesOffset + i * 8, value);
						total += value;
				...
				var color = getColor(vram, x1b, x1e, y1b, y1e, w, 1.0 / total, valuesOffset);
				vram.setInt32(outputOffset + (x + yRow) * 4, color);
			}
		}

		var result:Pixels = new Pixels(width, height, true);
		result.bytes = vram.sub(outputOffset, vramLength - outputOffset);
		return result;
	}

	function getColor(bytes:Bytes, x1b:Int, x1e:Int, y1b:Int, y1e:Int, w:Int, total:Float, valuesOffset:Int):Int
	{
		var i:Int = 0;
		...
		for (y1 in y1b...y1e)
		{
			...
			for (x1 in x1b...x1e)
			{
				var color:Int = bytes.getInt32((y2 + x1) * 4);
				var value = bytes.getDouble(valuesOffset + i * 8) * total;
		...
		var color:Int = _a << 24 | _r << 16 | _g << 8 | _b;
...
class MitchellKernel implements IKernel
{
	...
	public function new(b:Float = (1.0 / 3.0), c:Float = (1.0 / 3.0))
	{
		B = b;
		C = c;
		P0 = ((6.0 - 2.0 * B) / 6.0);
